...
#   -------------------------------------------------------------
#
#   Copied from: https://github.com/henryspatialanalysis/OpenPOIs
#   Source path: src/openpois/io/overture.py
#   Commit: e9c5c4f72da0c05696623510e1cdab3c26d75949
#
#   Kept in this repo at lib/overture.py so that updates can be pulled from
#   the upstream OpenPOIs repo directly.
#   -------------------------------------------------------------
...
def _finalize_snapshot_in_duckdb(
    ...
    """
    Combine all per-part intermediates into the final GeoParquet.

    Per-part files are already coarse-bbox filtered during download.  A second
    polygon ST_Within pass is intentionally skipped here to avoid a cross-join
    OOM on machines with limited RAM.  Area-level bbox clipping is applied later
    at conflation time (flows/02_silver_conflation.py) so no real data loss occurs.

    The output is readable by ``gpd.read_parquet`` with CRS EPSG:4326.
    """
    ...
    query = f"""
        COPY (
            SELECT
                ...
                overture_addr_city,
                overture_addr_state,
                overture_addr_postcode,
                overture_addr_country,
                overture_websites,
                overture_phones,
                overture_socials,
                ST_Point(longitude, latitude) AS geometry
            FROM read_parquet('{parts_glob}', union_by_name = true)
...
def download_overture_snapshot(
    ...
    print("Combining parts into final GeoParquet (bbox-filtered per part, ST_Within skipped)...")
